...
//#define TEST_VORONOI
//#define TEST_INFLUENCED
//#define FUN_FIREWORKS
...
void main() {
	...
	for(int i = 0; i < int(bezier_data[0]); ++i) {
		...
		bezier_crosses_bez.h1 = beziers[int(bezier_data[1])+4*i+1];
		bezier_crosses_bez.a2 = beziers[int(bezier_data[1])+4*i+2];
		bezier_crosses_bez.h2 = beziers[int(bezier_data[1])+4*i+3];
	...
	for(int i = 0; i < color_count; ++i) { c_s[i] = 3*i; }
...
#ifdef TEST_VORONOI
	fragColor = vec4(colors[c_s[0]+1], colors[c_s[0]+2][0], 1); return;
#endif

#ifndef FUN_FIREWORKS
	if(color_count >= 3) {
	...
	fragColor = vec4(fragColor.rgb/final_color_count, 1);
	return;
#endif
	for(int i = 0; i < color_count; ++i) {
		if(c_s[i] == -1) continue;
	...
	switch(final_color_count) {
		case 1:
			fragColor = vec4(colors[c_s[0]+1], colors[c_s[0]+2][0], 1);
			break;
		case 2:
			...
			fragColor = vec4(
				...
				(1-t)*colors[c_s[0]+2][0]+t*colors[c_s[1]+2][0],
			1);
			break;
		default:
			float weights = 0;
			...
			while(true) {
				if(colors[c_s[center]] == position) {
					fragColor = vec4(colors[c_s[center]+1], colors[c_s[center]+2][0], 1);
					return;
				}
			...
			fragColor = vec4(fragColor.xyz/weights, 1);
	}
}
)"
